#ifndef __USBD_CLASS_MSD_H_
#define	__USBD_CLASS_MSD_H_

#include <stdint.h>
#include <usbd_desc_common.h>
...
typedef struct
{
    ...
    uint8_t *read_buf;
    uint32_t (*usbd_msc_write)(uint32_t block, uint8_t *buf, uint32_t num_of_blocks);
    uint32_t (*usbd_msc_read)(uint32_t block, uint8_t *buf, uint32_t num_of_blocks);
}usbd_msd_t;

#define USBD_MSD_EP_BULKIN      (3)
#define USBD_MSD_EP_BULKOUT     (4)
...
/* MSD API */
void usbd_msd_init(usbd_msd_t *ops);
